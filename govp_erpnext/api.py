import frappe

from govp_erpnext.client import GovpExchangeClient


@frappe.whitelist()
def test_connection(company):
    frappe.only_for("System Manager")
    settings = frappe.get_doc("GOVP Company Settings", company)
    token = settings.get_password("connector_token")
    if not token:
        frappe.throw("Añada el token del conector antes de comprobar la conexión")
    result = GovpExchangeClient(settings.exchange_url, token).inspect()
    return {"ok": True, "connector": result.get("connector", result)}
